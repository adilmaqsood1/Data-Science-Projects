...
image = Image.open('Crypto-Price-App/logo.jpg')
...
def load_data():
    ...
    if data is not None:
        ...
        if 'props' in coin_data and 'initialState' in coin_data['props']:
            crypto_data = coin_data['props']['initialState'].get('cryptocurrency')
            if crypto_data and 'listingLatest' in crypto_data and 'data' in crypto_data['listingLatest']:
                listings = crypto_data['listingLatest']['data']
    ...
    # Return an empty DataFrame if the structure is not as expected or data is not available
    return pd.DataFrame()
